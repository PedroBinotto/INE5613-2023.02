...
public class LotacaoController {
    ...
    public ResponseEntity<List<LotacaoResponseDto>> getLotacoes(
        ...
        try {
            val funcionarioFilter = funcionarioId
                    .map(value -> Arrays.stream(value).collect(Collectors.toList()))
                    .orElseGet(ArrayList::new);
            val estabelecimentoFilter = estabelecimentoId
                    .map(value -> Arrays.stream(value).collect(Collectors.toList()))
                    .orElseGet(ArrayList::new);
            val cargoFilter = cargo
                    .map(value -> Arrays.stream(value).map(CargoEnum::getNome).collect(Collectors.toList()))
                    .orElseGet(ArrayList::new);
            return ResponseEntity.ok(this.lotacaoMapper.toDto(this.lotacaoQuery.getLotacoes(
                    funcionarioFilter,
    ...
    @Operation(summary = "GET lotacao BY funcionario ID, estabelecimento ID, cargo", tags = "lotacoes")
    public ResponseEntity<LotacaoResponseDto> getLotacaoById(
        ...
        try {
            return ResponseEntity.ok(this.lotacaoMapper.toDto(this.lotacaoQuery.getLotacaoById(
                    funcionarioId,
                    estabelecimentoId,
                    cargo.getId()
            )));
    ...
    @Operation(summary = "DELETE lotacao BY funcionario ID, estabelecimento ID, cargo", tags = "lotacoes")
    public ResponseEntity<Void> deleteLotacaoById(
            @PathVariable Long funcionarioId,
            @PathVariable Long estabelecimentoId,
            @PathVariable CargoEnum cargo
    ) {
        this.lotacaoQuery.deleteLotacaoById(
                funcionarioId,
                estabelecimentoId,
                cargo.getId()
        );
