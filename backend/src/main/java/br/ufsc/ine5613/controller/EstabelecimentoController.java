...
public class EstabelecimentoController {
  ...
  public ResponseEntity<List<EstabelecimentoResponseDto>> getEstabelecimentos(
    ...
    try {
      Long[] ufFilter =
          uf.map(
                  ufEnums ->
                      Arrays.stream(ufEnums).map(UfEnum::getId).toList().toArray(new Long[] {}))
              .orElseGet(() -> new Long[] {});
      return ResponseEntity.ok(
          this.estabelecimentoMapper.toDto(
  ...
  public ResponseEntity<EstabelecimentoResponseDto> getEstabelecimentoById(
    ...
    return ResponseEntity.ok(
        this.estabelecimentoMapper.toDto(
            this.estabelecimentoQuery.getEstabelecimentoById(estabelecimentoId)));
  }

  @PostMapping()
  @Operation(summary = "SAVE estabelecimento", tags = "estabelecimentos")
  public ResponseEntity<Void> saveEstabelecimento(
  ...
  public ResponseEntity<Void> updateEstabelecimentoById(
      @PathVariable Long estabelecimentoId, @RequestBody EstabelecimentoSaveDto estabelecimento) {
