...
public abstract class VendaMapper {
    ...
    void afterResponseDtoMapping(
            VendaDetailCompositeDto composite,
             @MappingTarget VendaResponseDto response
    ) {
      response.setLotacao(
      ...
      response.setCliente(new ClienteDetailCompositeDto(
             composite.getClienteId(),
             composite.getClienteCpf(),
             composite.getClienteNome(),
             composite.getClienteSobrenome()
      ));
      response.setProdutos(composite
              .getProdutos()
              .stream()
              .map(it ->
